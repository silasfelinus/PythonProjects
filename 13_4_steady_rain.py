...
class Raindrop(Sprite):
	...
	def __init__(self, screen):
		"""Initialize the raindrop and set it's starting positon"""
	...
	while True:
		...
		for raindrop in raindrops:
			...
			if raindrop.rect.bottom > screen_height:
				#Remove raindrop and add a new drop at a random spot at the top of the screen
				raindrops.remove(raindrop)
				new_raindrop = Raindrop(screen)
				raindrop.x = randint(0, screen_width)
				raindrop.rect.x = raindrop.x
				raindrop.rect.y = 0
				raindrops.add(raindrop)
			raindrop.blitme()
