...
class Ball(Sprite):
	...
	def __init__(self, screen, screen_width, screen_height):
		...
		#direction going up (positive) or down (negative)
		self.direction = 1
...
def initialize_dynamic_settings(ship, ball):
	ship.speed_factor = 1.5
	ball.speed_factor = 1

def increase_speed (ship, ball):
	speedup_scale = 1.1
	ship.speed_factor *= speedup_scale
	ball.speed_factor *= speedup_scale
...
def run_game():
	...
	while True:
			...
			if event.type == pygame.QUIT:
				...
				if play_button.rect.collidepoint(mouse_x, mouse_y):
					...
					if button_clicked and not game_active:
						...
						game_active = True
						lives = 3
						initialize_dynamic_settings(ship, ball)
		...
		if game_active == True:
			...
			ball.rect.y += (ball.speed_factor * ball.direction)
