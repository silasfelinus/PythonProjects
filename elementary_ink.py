...
def line_limit(fake, real):
	"""compare line limits and warn if there isn't enough room"""
	num_blanks = 0
	num_real = 0
	for line in fake:
		if line == '':
	...
	print("\nNumber of blank lines in fake message = {}\n".format(num_blanks))
	print("Number of lines in real message = {}\n".format(num_real))
	if num_real > num_blanks:
		print("Fake message needs {} more blank lines.".format(diff), file=sys.stderr)
		sys.exit()
